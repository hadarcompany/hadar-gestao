...
export default function MeuTrabalhoPage() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      if (filterDate) {
        ...
        data = data.filter((t: TaskData) => {
          if (!t.dueDate) return false;
          const due = new Date(t.dueDate);
          if (filterDate === "today") return due.toDateString() === today.toDateString();
          if (filterDate === "week") {
  ...
  // Lógica para separar ativas e calcular o widget
    ...
    tasks.forEach(task => {
      ...
      if (isCompleted) {
        ...
        if (task.dueDate) {
          ...
          calcStats.upcoming++; // Sem data conta como próxima
        }
      }
    });
        ...
        <div className="xl:col-span-3 space-y-3">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center bg-zinc-900/40 border border-zinc-800/50 rounded-2xl">
              <h3 className="text-lg font-medium text-white mb-2">Nenhuma tarefa atribuída a você</h3>
              <p className="text-sm text-zinc-500">Suas tarefas pendentes aparecerão aqui.</p>
            </div>
          ) : (
                    ...
                    <div className="flex items-center gap-2 text-zinc-400 min-w-[90px] justify-end">
                      <div className="hidden md:block text-xs text-right">
                        <span className={isUrgent ? "text-red-400 font-medium" : ""}>
                          {task.dueDate ? new Date(task.dueDate).toLocaleDateString("pt-BR", { day: '2-digit', month: '2-digit' }) : "—"}
                        </span>
                      </div>
